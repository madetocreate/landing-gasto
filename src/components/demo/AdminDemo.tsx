...
export function AdminDemo() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h3 className="text-xl font-semibold text-foreground mb-2">{t(locale, 'pages.demo.admin.title')}</h3>
        <p className="text-foreground-muted">{t(locale, 'pages.demo.admin.subtitle')}</p>
      </div>
      ...
      <div className="grid gap-[var(--space-6)] md:grid-cols-4">
        ...
        <div className="md:col-span-3">
          {active === 'setup' && (
            <SpotlightCard className={classNames(radiusCard, padCard)}>
              <div className="flex items-center justify-between gap-[var(--space-4)]">
                <div>
                  <h4 className="text-lg font-semibold text-foreground">{t(locale, 'pages.demo.admin.setup.h4')}</h4>
                  <p className="mt-1 text-sm text-foreground-muted">{t(locale, 'pages.demo.admin.setup.p')}</p>
                </div>
              ...
              <div className="mt-[var(--space-6)] space-y-[var(--space-3)]">
                ...
                <p className="text-sm text-foreground-muted">{t(locale, 'pages.demo.admin.setup.progress')}</p>
              </div>
            </SpotlightCard>
          )}
          ...
          {active === 'operations' && (
            <SpotlightCard className={classNames(radiusCard, padCard)}>
              <h4 className="text-lg font-semibold text-foreground">{t(locale, 'pages.demo.admin.operations.h4')}</h4>
              <p className="mt-1 text-sm text-foreground-muted">{t(locale, 'pages.demo.admin.operations.p')}</p>
                ...
                {(['open', 'inKitchen', 'paid'] as const).map((k) => (
                  ...
                  >
                    <div className="text-xs font-semibold text-foreground-muted uppercase tracking-wider">
                      {t(locale, `pages.demo.admin.operations.cards.${k}.label`)}
                    </div>
                    ...
                    <div className="mt-[var(--space-2)] text-xs text-foreground-muted">
                      {t(locale, `pages.demo.admin.operations.cards.${k}.hint`)}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {active === 'settings' && (
            <SpotlightCard className={classNames(radiusCard, padCard)}>
              <h4 className="text-lg font-semibold text-foreground mb-[var(--space-4)]">
                {t(locale, 'pages.demo.admin.settings.h4')}
              </h4>
              <div className="space-y-[var(--space-3)]">
                <div className="flex items-center justify-between py-[var(--space-3)]">
                  <span className="text-sm text-foreground">{t(locale, 'pages.demo.admin.settings.rows.languages')}</span>
                ...
                <div className="flex items-center justify-between py-[var(--space-3)] border-t border-border/50">
                  <span className="text-sm text-foreground">{t(locale, 'pages.demo.admin.settings.rows.currency')}</span>
                  <span className="text-sm text-foreground-muted">EUR</span>
                </div>
                <div className="flex items-center justify-between py-[var(--space-3)] border-t border-border/50">
                  <span className="text-sm text-foreground">{t(locale, 'pages.demo.admin.settings.rows.timezone')}</span>
          ...
          {active === 'kds' && (
            <SpotlightCard className={classNames(radiusCard, padCard)}>
              <h4 className="text-lg font-semibold text-foreground mb-[var(--space-4)]">{t(locale, 'pages.demo.admin.kds.h4')}</h4>
              <p className="text-sm text-foreground-muted">{t(locale, 'pages.demo.admin.kds.p')}</p>
                ...
                {(['kitchen', 'bar'] as const).map((station) => (
                  ...
                  >
                    <span className="text-sm font-semibold text-foreground">
                      {t(locale, `pages.demo.admin.kds.stations.${station}`)}
                    </span>
                    <span className="text-xs text-foreground-muted">{t(locale, 'pages.demo.admin.kds.statusActive')}</span>
                  </div>
                ))}
              </div>
            </SpotlightCard>
