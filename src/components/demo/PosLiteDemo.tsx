    ...
    <div className="grid gap-[var(--space-6)] lg:grid-cols-3">
      <SpotlightCard className="p-[var(--space-6)] lg:col-span-2">
        <div className="flex items-start justify-between gap-[var(--space-4)]">
          <div>
            <h4 className="text-lg font-semibold text-foreground">{t(locale, "pages.demo.admin.pos.floor.h4")}</h4>
            <p className="mt-1 text-sm text-foreground-muted">{t(locale, "pages.demo.admin.pos.floor.p")}</p>
          </div>
          <div className="text-xs text-foreground-muted">{t(locale, "pages.demo.admin.pos.badge")}</div>
        </div>
          ...
          {tables.map((tb) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-[var(--space-2)] text-xs text-foreground-muted">{t(locale, "pages.demo.admin.pos.floor.tapHint")}</div>
              </button>
            );
          })}
        </div>
      ...
      <SpotlightCard className="p-[var(--space-6)]">
        <h4 className="text-lg font-semibold text-foreground">{t(locale, "pages.demo.admin.pos.cashier.h4")}</h4>
        <p className="mt-1 text-sm text-foreground-muted">{t(locale, "pages.demo.admin.pos.cashier.p")}</p>
        ...
        <div className="mt-[var(--space-6)] rounded-[var(--radius-2xl)] border border-border/50 bg-background-muted/40 p-[var(--space-4)]">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold text-foreground">
              {t(locale, "pages.demo.admin.pos.cashier.activeTable")}: {active.label}
            </div>
          ...
          <div className="mt-[var(--space-4)] flex items-center justify-between border-t border-border/40 pt-[var(--space-3)]">
            <span className="text-sm font-semibold text-foreground">{t(locale, "pages.demo.admin.pos.cashier.total")}</span>
        ...
        <div className="mt-[var(--space-4)] grid grid-cols-2 gap-[var(--space-3)]">
          ...
          >
            {t(locale, "pages.demo.admin.pos.cashier.actions.split")}
          </button>
          <button
            type="button"
          ...
          >
            {t(locale, "pages.demo.admin.pos.cashier.actions.pay")}
          </button>
        </div>
      </SpotlightCard>
    </div>
